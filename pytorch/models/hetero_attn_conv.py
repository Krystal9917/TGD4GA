...
class SHANConv(MessagePassing):
    ...
    def forward(
            ...
            x_score_dict: Dict[NodeType, Tensor],
    ...
    def message(self, x_j, alpha_i, alpha_j, index, edge_weight, ptr, size_i):
        alpha = alpha_j + alpha_i
        alpha = F.leaky_relu(alpha, self.negative_slope)
        alpha = softmax(alpha, index, ptr, size_i)
        alpha = F.dropout(alpha, p=self.dropout, training=self.training)
        weight_alpha = edge_weight * alpha
        out = x_j * weight_alpha.view(-1, self.heads, 1)
