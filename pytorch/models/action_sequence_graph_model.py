...
class SeqGraphUin2Uin(nn.Module):
    ...
    def __init__(self, target_feat, vocab_size, max_len, n_layers=2, emb_dim=512, n_heads=2, output_size=11,
        ...
        self.wd = WideDeepNet(target_feat, emb_dim // 2)
        self.encoder = Encoder(n_heads, emb_dim, drop_rate, n_layers)
        self.flatten = nn.Flatten(start_dim=1)
        self.o = nn.Linear(max_len * emb_dim, output_size)

    def forward(self, x, target_x, training=None):
        # print(" x.size()", x.size())
        # print("target_x", target_x.size())
        x_embed = self.embeddings(x)
        # 行为对象的特征矩阵逐行过w&d模型
        target_x_embed = self.wd(target_x.view(-1, target_x.size(2))).view(target_x.size(0), target_x.size(1), -1)
        # 特征fusion
