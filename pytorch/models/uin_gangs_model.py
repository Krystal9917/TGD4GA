...
class WideDeepNet(nn.Module):
    def __init__(self, uin_in_size, output_size, drop_rate):
        super(WideDeepNet, self).__init__()
        self.wide = nn.Linear(uin_in_size, output_size)
        self.deep = nn.Sequential(
            nn.Linear(uin_in_size, uin_in_size * 2),
            nn.Dropout(drop_rate),
            nn.LeakyReLU(),
            nn.Linear(uin_in_size * 2, output_size * 2),
            nn.Dropout(drop_rate),
            nn.LeakyReLU(),
