...
class MaskRGCNLayer(torch.nn.Module):
    ...
    def forward(self, x, edge_index, edge_type, score):
        if self.is_embed:
            x = self.mlp_w(x)
        x = self.conv(x, edge_index, edge_type, score)
        return x


class MaskRGCN(torch.nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim, num_relations, mlp_n_in_dim,
                 mlp_c_in_dim, mlp_t_in_dim=None, is_dropout=False, adapter_type=None,
                 metric_learning='similarity', num_bases=10, is_embed=False, layer=2,
                 first_attn_weight='split'):
        super().__init__()
        self.layer = layer
        for i in range(self.layer):
            if i == 0:
                ...
                self.conv2 = MaskRGCNLayer(hidden_dim, output_dim, num_relations,
                                           mlp_n_in_dim, mlp_c_in_dim, mlp_t_in_dim=mlp_t_in_dim,
                                           adapter_type=adapter_type, attn_weight_type='all',
                                           is_dropout=is_dropout, metric_learning=metric_learning,
                                           num_bases=num_bases, is_embed=is_embed)

    def forward(self, x, edge_index, edge_type, score=None):
        x_out = []
        x = self.conv1(x, edge_index, edge_type, score)
        x_out.append(x)
        if self.layer == 2:
            x = self.conv2(x, edge_index, edge_type, score)
            x_out.append(x)
        return x_out
