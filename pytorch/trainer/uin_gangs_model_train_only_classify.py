...
from mmgog_long_term_sequence_model.pytorch.models.LossFunction import InfoNCELoss, InfoNCELossV2, InfoNCELossV3, \
    WeightedFocalBalanceBCELoss, WeightedFocalLoss, CosineEmbeddingLossModule, MultiLabelClassifyBaseBCE
...
class UinGangsModelTrainOnlyClassify:
    ...
    def __init__(self, args_dict):
        ...
        self.classify_criterion = MultiLabelClassifyBaseBCE(
            alpha=0.25,
            gamma=2.0,
            reduction='mean',
            target_ratio=5,
            weights=self.train_data.label_class_weight.to(self.device))
        self.optimizer = torch.optim.Adam(list(self.model.parameters()) + list(self.classify_criterion.parameters()),
    ...
    def train(self):
        ...
        for epoch in range(self.train_dict["n_epochs"]):
            ...
            for step, batch_data in enumerate(self.train_loader):
                ...
                classify_loss = self.classify_criterion(batch_root_classify_emb, batch_label_one_hot)
            ...
            with torch.no_grad():
                for step, batch_data in enumerate(self.test_loader):
                    ...
                    batch_root_emb = torch.stack(root_node_emb_list).to(self.device)
                    batch_root_classify_emb = torch.stack(root_node_classify_out_list).to(self.device)
                    y_pred.extend(torch.sigmoid(batch_root_classify_emb).detach().cpu().numpy().tolist())
                    # similarity_loss = self.criterion(batch_root_emb, batch_label)
                    classify_loss = self.classify_criterion(batch_root_classify_emb, batch_label_one_hot)
                    # loss = 5 * classify_loss + similarity_loss
                    loss = classify_loss
            ...
            thresholds = [0.6, 0.6, 0.6, 0.6, 0.6, 0.9, 0.6, 0.95, 0.95, 0.9, 0.6, 0.8, 0.9, 0.9, 0.6, 0.6, 0.9, 0.9,
                          0.6]
            ...
            if np.mean(test_classify_loss) < self.train_dict["best_loss"]:
                self.save_model_states()
                logger.info('uin_gangs_model states saved')
