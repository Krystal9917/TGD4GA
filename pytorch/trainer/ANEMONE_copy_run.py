...
from mmgog_long_term_sequence_model.utils.ANEMONE_utils import load_mat, preprocess_features, adj_to_dgl_graph, normalize_adj, generate_rwr_subgraph
    ...
    for run in range(args.runs):
        ...
        for epoch in range(args.num_epoch):
            ...
            if mean_loss < best:
                ...
                best_t = epoch
                cnt_wait = 0
                torch.save(model.state_dict(), '/chongqinggeminiceph1fs/geminicephfs/security-others-common/jiujiuchen'
                                               '/projects/mmgog_long_term_sequence_model/data/saved_model/checkpoints'
                                               '/exp_{}.pkl'.format(args.expid))
            else:
                cnt_wait += 1
        ...
        model.load_state_dict(torch.load('/chongqinggeminiceph1fs/geminicephfs/security-others-common/jiujiuchen'
                                         '/projects/mmgog_long_term_sequence_model/data/saved_model/checkpoints'
                                         '/exp_{}.pkl'.format(args.expid)))
